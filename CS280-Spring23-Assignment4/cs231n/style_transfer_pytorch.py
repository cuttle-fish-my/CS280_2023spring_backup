...
def content_loss(content_weight, content_current, content_original):
    ...
    assert _ == 1, f"The first dimension of content should be 1 but get {_}"
    F = content_current.reshape(C, H * W)
    P = content_original.reshape(C, H * W)
    loss = content_weight * ((F - P) ** 2).sum()
    return loss
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    loss = 0
    for idx, target, w in zip(style_layers, style_targets, style_weights):
        loss += w * ((gram_matrix(feats[idx].clone()) - target) ** 2).sum()
    return loss
...
def tv_loss(img, tv_weight):
    ...
    N, C, H, W = img.shape
    assert N == 1 and C == 3, f"Invalid shape {img.shape}"
